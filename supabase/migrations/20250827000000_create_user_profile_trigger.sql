-- Migration pour créer automatiquement un profil utilisateur lors de l'inscription
-- Fichier: 20250827000000_create_user_profile_trigger.sql
...
BEGIN
  ...
  -- Créer également une entrée dans user_activities pour le tracking
...
-- Fonction pour mettre à jour automatiquement le profil lors de modifications
CREATE OR REPLACE FUNCTION public.handle_user_update()
RETURNS TRIGGER AS $$
BEGIN
  -- Mettre à jour le profil si l'email ou les métadonnées changent
...
-- Fonction pour nettoyer les données lors de la suppression d'un utilisateur
...
-- Créer le trigger pour la suppression d'utilisateurs
...
COMMENT ON FUNCTION public.handle_user_delete() IS 'Nettoie toutes les données associées lors de la suppression d''un utilisateur';

-- Créer une fonction utilitaire pour obtenir les statistiques utilisateur
