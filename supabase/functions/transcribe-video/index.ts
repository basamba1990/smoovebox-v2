...
Deno.serve(async (req) => {
  ...
  try {
      ...
      try {
        ...
        try {
          ...
          if (publicUrlData?.publicUrl) {
            ...
            console.log(`URL publique générée: ${videoUrl}`)
          } else {
            throw new Error('Impossible de générer une URL publique')
          }
        } catch (publicUrlError) {
          console.error('Erreur lors de la génération de l\'URL publique:', publicUrlError)
          
          // Dernier recours: construire manuellement l'URL
          videoUrl = buildCompleteUrl(supabaseUrl, bucket, filePath)
          console.log(`URL construite manuellement: ${videoUrl}`)
        }
      }
    }

    if (!videoUrl) {
      console.error('Impossible de déterminer l\'URL de la vidéo')
    ...
    try {
      ...
      // Valider les données de transcription
      const validation = validateTranscriptionData(transcription);
      if (!validation.isValid) {
        console.warn('Données de transcription invalides:', validation.errors);
        // Continuer malgré les avertissements, mais les logger
      }
    ...
    // Calculer le score de confiance moyen si des segments sont disponibles
    let confidenceScore = null;
    if (transcription.segments && Array.isArray(transcription.segments) && transcription.segments.length > 0) {
      const validSegments = transcription.segments.filter((seg: any) => 
        seg.avg_logprob !== undefined && seg.avg_logprob !== null && !isNaN(seg.avg_logprob)
      );
      
      if (validSegments.length > 0) {
        confidenceScore = validSegments.reduce((acc: number, seg: any) => acc + seg.avg_logprob, 0) / validSegments.length;
        console.log(`Score de confiance calculé: ${confidenceScore} (basé sur ${validSegments.length} segments)`);
      }
    }

    // Insérer dans la table transcriptions avec gestion d'erreur améliorée
    try {
      const { error: transcriptionTableError } = await serviceClient
        ...
        .upsert({
          ...
          segments: transcription.segments || [],
          duration: transcription.duration || null, // Cette colonne existe maintenant grâce à la migration
      ...
      if (transcriptionTableError) {
        ...
        return new Response(
          JSON.stringify({ 
            ...
            event_message: `Erreur lors de l'enregistrement de la transcription: ${JSON.stringify(transcriptionTableError, null, 2)}`
          }),
          { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 500 }
        )
      }
      ...
      console.error('Exception lors de l\'insertion de la transcription:', insertError)
      
      await serviceClient
        .from('videos')
        .update({ 
          status: VIDEO_STATUS.FAILED, 
          error_message: `Exception lors de l'enregistrement: ${insertError.message}`,
          updated_at: new Date().toISOString()
        })
        .eq('id', videoId as string)
      ...
      return new Response(
        JSON.stringify({ 
          ...
          event_message: `Exception lors de l'enregistrement de la transcription: ${JSON.stringify(insertError, null, 2)}`
        }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 500 }
      )
    }
    ...
    if (videoUpdateError) {
      ...
      console.log('Table videos mise à jour avec succès.');
    }
    ...
    return new Response(
      JSON.stringify({
        ...
        event_message: `Erreur générale dans la fonction transcribe-video: ${JSON.stringify(error, null, 2)}`
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 500 }
    )
  }
