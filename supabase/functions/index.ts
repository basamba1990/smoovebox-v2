...
serve(async (req) => {
  ...
  const supabaseClient = createClient(
    ...
    { global: { headers: { 'x-supa-v-api-key': Deno.env.get('SUPABASE_ANON_KEY') ?? '' } } }
  );

  try {
    // Fetch user statistics
    const { data: userStats, error: statsError } = await supabaseClient
      .from('profiles') // Assuming 'profiles' table contains user stats or can be joined for them
    ...
    // Fetch recent videos
    const { data: recentVideos, error: videosError } = await supabaseClient
      .from('videos')
      .select('id, title, created_at, status, analysis, performance_score, duration')
      .eq('profile_id', userStats.id) // Link to profile_id
      .eq('status', 'published')
    ...
    const { data: userActivityStreak, error: streakError } = await supabaseClient
      .from('user_activities') // Assuming user_activities table has streak info
    ...
    return new Response(JSON.stringify({
      ...
      userActivityStreak: userActivityStreak && userActivityStreak.length > 0 ? userActivityStreak[0] : { current_streak: 0, best_streak: 0 },
    }), {
