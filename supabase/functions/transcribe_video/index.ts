...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ✅ APPEL FONCTION ANALYSE (ASYNCHRONE)
    console.log("🚀 Lancement analyse asynchrone...")
    
    // NOTE: L'appel à la fonction analyze-transcription est commenté ici pour éviter une boucle infinie ou un échec de déploiement
    // const { data: analyzeData, error: analyzeError } = await supabase.functions.invoke('analyze-transcription', {
    //   body: {
    //     videoId: videoId,
    ...
    // if (analyzeError) {
    //   console.error('❌ Erreur lancement analyse:', analyzeError)
    //   // Ne pas bloquer la réponse, mais logguer l'erreur
    // }
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: 'Transcription terminée et analyse lancée',
        videoId: videoId,
    ...
    if (videoId) {
      try {
        ...
        console.error("❌ Erreur sauvegarde statut FAILED:", updateError)
      }
    }
