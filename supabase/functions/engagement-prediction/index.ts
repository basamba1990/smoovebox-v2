...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [
      ...
      // Données de la vidéo
      supabase.from('videos').select('*, user_id, view_count, like_count').eq('id', videoId).single(),
    ...
    const gamification = {
      badges_disponibles: [
        { id: "first_comment", nom: "Premier commentaire", description: "Laisse ton premier commentaire", difficulté: 1 },
        { id: "engagement_star", nom: "Star de l'engagement", description: "Interagis avec 5 vidéos consécutives", difficulté: 2 },
        { id: "feedback_master", nom: "Maître du feedback", description: "Laisse un commentaire constructif", difficulté: 3 }
      ],
      défis_quotidiens: [
      ...
      niveau_actuel: userId ? Math.floor(Math.random() * 5) + 1 : 0,
      points: userId ? Math.floor(Math.random() * 100) + 50 : 0,
      prochain_niveau_dans: userId ? Math.floor(Math.random() * 50) + 10 : null
    };
