...
import { useUser } from '@supabase/auth-helpers-react';  // Ajout pour state user synchrone
...
const RecordVideo = ({ onVideoUploaded = () => {} }) => {
  ...
  const [title, setTitle] = useState('Ma vidéo SpotBulle');  // Titre éditable
  ...
  const user = useUser();  // Hook pour user synchrone (évite await répétitif)
  ...
  // CORRECTION CRITIQUE : Uploader la vidéo avec file_path OBLIGATOIRE (await user)
  const uploadVideo = async () => {
    ...
    try {
      // 1. Upload fichier vers storage (path avec user.id)
      const fileName = `${Date.now()}.webm`;
      const { data: uploadData, error: uploadError } = await supabase.storage
        .from('videos')
        .upload(`${user.id}/${fileName}`, recordedVideo.blob, {
          upsert: true,
      ...
      // 2. Insert en DB avec storage_path et user_id
      const { data: videoData, error: insertError } = await supabase
        .from('videos')
        .insert({
          user_id: user.id,  // Synchrone via hook
          title: title || 'Sans titre',
          description: 'Vidéo enregistrée via SpotBulle',
          file_path: storagePath,
          storage_path: storagePath,
          status: VIDEO_STATUS.UPLOADED,
          duration: Math.round(recordingTime),  // Temps enregistrement
      ...
      await triggerTranscription(videoData.id, user.id, publicUrl);
  ...
  if (!user) {
    return <div>Chargement... (ou redirection login)</div>;  // Fallback
  }
