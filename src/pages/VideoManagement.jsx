...
const VideoManagement = () => {
  ...
  const fetchVideos = useCallback(async () => {
    ...
    try {
      ...
      const normalizedVideos = (data || []).map(video => {
        ...
        if (!Object.keys(analysisData).length && video.ai_result) {
          try {
      ...
      setSelectedVideo(prev => {
        if (!prev) return null;
        const updatedSelected = normalizedVideos.find(v => v.id === prev.id);
  ...
  const getPublicUrl = (video) => {
    ...
    try {
      ...
      if (!supabaseUrl) throw new Error('URL Supabase non configurée');
  ...
  const transcribeVideo = async (video) => {
    ...
    try {
      ...
      if (!videoUrl) throw new Error('URL de la vidéo non disponible');
      ...
      const response = await fetch(`${supabaseUrl}/functions/v1/transcribe-video`, {
        ...
        body: JSON.stringify({ videoId: video.id, videoUrl }),
      });
      
      if (!response.ok) {
        const errorResult = await response.json().catch(() => ({}));
        throw new Error(errorResult.error || `Erreur ${response.status}: ${response.statusText}`);
      }
      ...
      setVideos(prev =>
        prev.map(v =>
          v.id === video.id ? { ...v, status: 'transcribing', statusLabel: 'Transcription en cours' } : v
        )
      );
      setSelectedVideo(prev =>
        prev?.id === video.id ? { ...prev, status: 'transcribing', statusLabel: 'Transcription en cours' } : prev
      );
      
      setTimeout(() => {
        if (mountedRef.current) fetchVideos();
      }, 3000);
    } catch (err) {
      console.error('Erreur lors de la transcription:', err);
      let errorMessage = err.message || 'Erreur inattendue';
  ...
  const analyzeVideo = async (video) => {
    ...
    try {
      ...
      const response = await fetch(`${supabaseUrl}/functions/v1/analyze-transcription`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
      ...
      setVideos(prev =>
        prev.map(v =>
          v.id === video.id ? { ...v, status: 'analyzing', statusLabel: 'En analyse' } : v
        )
      );
      setSelectedVideo(prev =>
        prev?.id === video.id ? { ...prev, status: 'analyzing', statusLabel: 'En analyse' } : prev
      );
  ...
  const deleteVideo = async (video) => {
    ...
    try {
      ...
      if (transcriptionError) console.warn('Erreur lors de la suppression de la transcription:', transcriptionError);
